...
func TestSiteSettingsAdminHandlerUpdateSiteProfileSuccess(t *testing.T) {
	...
	ctx := newAdminTestContext(
		...
		`{"brand_text":" Miku Blog ","logo_alt":" Miku Blog logo ","site_title":" Miku Blog ","site_url":" https://nanamiku.blog ","default_description":" hello ","default_social_image":" /picture/social.png "}`,
	)
...
func TestSiteSettingsAdminHandlerUpdateSiteIntegrationsServiceError(t *testing.T) {
	...
	ctx := newAdminTestContext(
		...
		`{"github_username":"awkker","weather_location":"Tokyo","show_weather":true,"show_music":false,"show_clock":true}`,
	)
	ctx.Set("admin_id", uuid.New())
